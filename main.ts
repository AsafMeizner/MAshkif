import { app, BrowserWindow, Tray, nativeImage, Notification, ipcMain, screen, BrowserWindowConstructorOptions, IpcMainEvent } from 'electron';
...
function createTrayWindow(): void {
  const trayWindowOptions: BrowserWindowConstructorOptions = {
    width: 260,
    height: 100,
    show: false,
    frame: false,
  ...
  trayWindow = new BrowserWindow(trayWindowOptions);
...
function setupIpcHandlers(): void {
  ...
  ipcMain.on('tray-resize', (event: IpcMainEvent, height: number) => {
    if (trayWindow) {
